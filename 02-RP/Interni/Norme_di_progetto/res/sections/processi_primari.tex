...
\subsubsection{Scopo}
Lo scopo del processo di fornitura è quello di determinare le procedure e le risorse 
...
\noindent È compito del responsabile di progetto organizzare riunioni tra i membri del 
		...
		\item \textbf{Importanza}: ogni requisito può assumere uno dei seguenti 
			...
			\begin{itemize}
				...
				\item \textit{2}: requisito desiderabile, ovvero non strettamente 
					necessario ma a valore aggiunto riconoscibile;
...
\noindent Le parti principali sono due:
	\begin{itemize}
		\item \textbf{technology baseline}: ha lo scopo di verificare che tutte le tecnologie scelte siano compatibili fra loro e che esse siano adatte al soddisfacimento dei requisiti individuati nel documento \textit{Analisi dei Requisiti};
		\item \textbf{product baseline}: basandosi su ciò che è emerso dalla technology baseline\glo, viene progettata e codificata una soluzione atta a soddisfare tutti i requisiti selezionati. Inoltre definisce i test necessari alla verifica. \newline
	\end{itemize}


\paragraph*{Technology baseline} \mbox{}\\

\noindent I \textit{progettisti} ed i \textit{programmatori} dovranno progettare e codificare il Proof of Concept\glo, un prototipo volto a:
	\begin{itemize}
		\item \textbf{testare le tecnologie}: verificare che tutte le tecnologie selezionate riescano ad interagire fra loro;
		\item \textbf{garantire la completezza dell'architettura di base del sistema}: verificare che tutte le tecnologie implementate riescano a garantire la futura implementazione di tutte le funzionalità.
...
\noindent A carico dei \textit{progettisti} c'è anche la product baseline che si sofferma su 
diversi aspetti tra i quali:
	\begin{itemize}
		...
		\item \textbf{test di unità}: devono essere definiti al fine di verificare 
			...
			\item \textbf{test di integrazione}: l'unione delle parti, intese come classi di verifica, permette di verificare che ogni componente del sistema 
			funzioni nella maniera voluta.
	\end{itemize}
