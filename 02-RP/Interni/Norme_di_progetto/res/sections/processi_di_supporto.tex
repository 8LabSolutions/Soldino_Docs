...
\subsection{Gestione della qualità}
	...
	\begin{itemize}
		...
		\item le metriche.
	\end{itemize}
\begin{comment}	\subsubsection{Metriche per i processi}
	...
	\subsubsection{Processi di Supporto}
		...
		\begin{itemize}
			\item \textbf{Budget at Completion} (BAC): budget totale allocato per il progetto;
			\item \textbf{Actual Cost} (AC): il denaro speso fino al momento del calcolo;
			\item \textbf{Earned Value} (EV): metrica di utilità per il calcolo di $SV$ e $CV$ (spiegate successivamente). Si tratta del valore del lavoro fatto fino al momento del calcolo; corrisponde al denaro guadagnato fino a quel momento. Si misura con $BAC \cdot \%\ di\ lavoro\ completato\ $;
		...
		\end{itemize}
		La metrica utilizzata è la \textbf{Code Coverage} (CC), una percentuale che indica il numero di righe di codice percorse dai test durante la loro	esecuzione. Per linee di codice totali si intende tutte quelle appartenenti all’unità in fase di test e si clacola con la seguente formula:
		\begin{center}
		...
		L'obiettivo è di scrivere la documentazione affinche costituisca una body of knowledge\glosp che raccoglie la conoscenza in modo:
		\begin{itemize}
			\item completo;
			\item non ambiguo;
		...
		\end{itemize}
		Le metricche utilizzate sono:
		\begin{itemize}
		\item l'\textbf{Indice di Gunning fog}, ossia una stima degli anni di educazione scolastica richiesti per comprendere il testo a una prima lettura. Si misura con la seguente formula:
		\begin{center}
		$ I_{GF} = 0.4 \cdot
		(
	...
	\subsubsection{Processi Organizzativi}
		...
		Tale processo ha per scopo il raggiungimento di un grado soddisfacente di qualità nel progetto, fornendo così obiettivi da perseguire e strumenti tecnici come procedure e metriche.\\
		L'obiettivo è di garantire che i prodotti e processi rispettino gli standard di qualità richiesti.\\
		La strategia prevede di: 
		\begin{itemize}
		...
		\end{itemize}
		La metricha utilizzata è la \textbf{Percentuale di metriche soddisfatte}, calcolata dalla seguente formula:
		\begin{center}
			$\frac{numero\ di\ metriche\ soddisfatte}{numero\ di\ metriche\ totali} $
		\end{center}
\end{comment}
\subsection{Verifica}
	\subsubsection{Scopo}
