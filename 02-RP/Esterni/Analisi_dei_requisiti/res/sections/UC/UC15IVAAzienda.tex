...
\begin{itemize}
	...
	\begin{enumerate}[label=\alph*.]
		...
		\item del trimestre scelto viene visualizzata la lista delle fatture relative al trimestre selezionato;
	\end{enumerate}
...
\end{itemize} 
\subsubsection{UC15.2 - Visualizzazione lista fatture per trimestre}
\begin{itemize}
	\item \textbf{Attori Primari}: azienda;
...
\begin{itemize}
	\item \textbf{Attori Primari}: azienda;
	\item \textbf{Descrizione}: per ogni fattura l'utente può visualizzarne i dettagli di essa, precisamente:
	\begin{itemize}
	\item numero identificativo;
	\item data;
	...
	\begin{itemize}
		...
		\end{itemize}
	\end{itemize}
    \item lista dei prodotti acquistati, includendo:
    \begin{itemize}
    	\item descrizione del prodotto;
    	\item quantità acquistata;
    	\item prezzo unitario;
    	\item aliquota IVA;
    	\item totale imponibile;
    \end{itemize}
	\item importo totale dell'ordine;
	\item IVA a credito/debito derivante dalla transazione.
	\end{itemize}
	
	\item \textbf{Scenario principale}: l'utente dopo aver selezionato una fattura, visualizza i dettagli di essa descritti precedemtente;
	\item \textbf{Precondizione}: l'azienda ha selezionato la visualizzazione della lista fatture per trimestre; un'opzione, da lei selezionata, le ha permesso di visualizzare la fattura scendendo nei dettagli;
