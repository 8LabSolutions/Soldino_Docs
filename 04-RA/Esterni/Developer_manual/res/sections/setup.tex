...
Choose "Save file" and at the end of download go to the destination directory, launch \texttt{.msi} file and follow the instructions until the end.
...
Ganache-cli is the same things as the previous one, but you can use Ganache only by command line.
...
You have to clone the \textit{Soldino} repository on GitHub: open the command line, move to the directory where you want place Soldino, then use \texttt{git clone https://github.com/8LabSolutions/Soldino-PoC}. After that use the command: \texttt{cd Soldino-PoC} to move into the directory.
% and then use \texttt{git checkout final}.  

\subsubsection{Ganache GUI}
You have to go to the folder where you've put Ganache, and open it with a double click.
\begin{figure}
	\centering
...
\begin{itemize}
	\item run the command:     \texttt{truffle deploy ---network ropsten}. This command will check if there are any changes between the compiled local contract and the currently deployed version on the network. If so, it will migrate them in the network. This operation could last for some minutes;
...
\end{itemize}
These commands will deploy the contracts that are correctly deployed in the file inside the \texttt{migrations} folder, called \texttt{2\_deploy\_contracts.js}. In the case you update only some contracts, remember to update also the allowance between the contracts and the addresses in the contract-manager.

\subsection{Deploy to Surge}
...
\begin{itemize}
	...
	\item create an optimized build of the project by running the command \texttt{rpm run build} from the root folder;
